...
//     }
// })
const e=document.getElementById("removable");[...document.querySelectorAll('[data-bs-toggle="tooltip"]')].map((e=>new bootstrap.Tooltip(e)));e?.addEventListener("click",(t=>{const o=t.target;if("button"===o.type){const t=document.getElementById("modalContent"),n=/(\d{1,2})/g,m=o.id.match(n);if(m){const o=m[0],n=document.getElementById("confirmRemove");e.classList.contains("admin")?n.href=`/admin/book/remove/${o}`:n.href=`/cart/remove/${o}?quantity=1`;const c=document.getElementById(`book-${o}`).innerText;console.log(`Book name: ${c}`),t.innerText=`${c}`}}}));